(ns ideal_sweden.main
  "Kotodama WASM entrypoint for the ideal_sweden clean-room actor (L4) — Clojure port.
  ...
  facts (`ideal_sweden.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ideal_sweden")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ideal_sweden.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ideal_sweden-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ideal_sweden-compat/ready)
